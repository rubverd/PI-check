...
def evaluate(context: MastgEvaluationContext) -> MastgRuleResult:
    if not context.has_mobsf_json:
        return MastgRuleResult.not_evaluable(
            "No hay informe MobSF JSON disponible para aplicar la regla PI-check de backup."
        )
    ...
    details = {
        ...
        "policy": "PI-check considera de alto riesgo allowBackup=true sin reglas restrictivas claras en apps mHealth.",
    }
    ...
    if allow_backup_true and not backup_rules:
        return MastgRuleResult.fail(
            "La aplicación permite backup y no se han identificado reglas restrictivas claras para excluir datos sensibles.",
            details=details,
            evidence=evidence,
            recommendation=(
                "Deshabilitar backup o definir reglas explícitas para excluir credenciales, tokens, "
                "datos personales y datos de salud."
            ),
        )

    if allow_backup_true and backup_rules:
        return MastgRuleResult.review(
            "La aplicación permite backup y declara reglas de backup/extracción; requiere validar que excluyen datos sensibles.",
            details=details,
            evidence=evidence,
            recommendation=(
                "Revisar fullBackupContent/dataExtractionRules para confirmar que no se respaldan datos sensibles."
            ),
        )
